...
class DiligenceFlow(Flow[DiligenceState]):
    ...
    async def organize_questionnaire_data(self) -> dict:
        ...
        clean_questionnaire_data = await organizer_agent.kickoff_async(query, response_format=OrganizedData)
        #clean_questionnaire_data = result.pydantic.data if result.pydantic else {}
    ...
    async def quality_check_organized_data(self, clean_questionnaire_data: dict) -> OrganizerFeedback:
        query = (
            ...
            f"Return the feedback on the data quality and whether it is acceptable or needs re-processing"
        )
        result = await organizer_agent.kickoff_async(query, response_format=OrganizerFeedback)
        feedback = result.pydantic if result.pydantic else OrganizerFeedback(feedback="Error getting feedback", is_acceptable=False)
